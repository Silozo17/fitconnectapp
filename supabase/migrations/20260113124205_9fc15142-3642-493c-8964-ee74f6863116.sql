-- Fix get_simple_coaches: drop existing function first to change return type
-- Root cause: column cb.expires_at does not exist in coach_boosts table
-- Fix: remove boost join entirely, set is_sponsored = FALSE (no boost logic)

-- Drop the existing function (all overloads with this signature)
DROP FUNCTION IF EXISTS public.get_simple_coaches(TEXT, TEXT, TEXT[], NUMERIC, NUMERIC, BOOLEAN, BOOLEAN, INTEGER);

-- Recreate with fixed body (no coach_boosts join)
