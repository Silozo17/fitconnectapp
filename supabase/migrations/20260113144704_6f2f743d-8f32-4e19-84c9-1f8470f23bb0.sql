-- Create get_filtered_coaches_v1: Filters ONLY, NO ranking
-- Preserves existing order (created_at DESC) like get_simple_coaches
...
BEGIN
  ...
  WHERE
    ...
    -- City filter (from Google API)
    AND (
      p_city IS NULL
      OR LOWER(TRIM(cp.location_city)) = LOWER(TRIM(p_city))
    )
    
    -- Region filter (from Google API)
    AND (
      p_region IS NULL
    ...
    -- Price filter (FIXED: handles NULL hourly_rate correctly)
    AND (
    ...
    -- Availability filter (CORRECT LOGIC)
    AND (
