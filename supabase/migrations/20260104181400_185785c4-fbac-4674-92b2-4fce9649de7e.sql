-- Update get_ranked_coaches with distance-based ranking and Amazon-style boost
-- Key changes:
-- 1. Add p_user_lat and p_user_lng parameters for distance calculation
-- 2. Calculate distance_miles using Haversine formula when coordinates available
-- 3. Create proximity tiers based on distance (0-3mi, 3-6mi, 6-10mi, 10-20mi, 20+mi)
-- 4. Online-only coaches get tier 6 (shown after nearby in-person coaches)
-- 5. ORDER BY: tier first (proximity), then is_sponsored (boost within tier), then score, then id (stable)
-- 6. Deterministic tiebreaker (rc.id ASC) prevents visual reordering
...
BEGIN
  ...
  ranked_coaches AS (
    SELECT 
      ...
      -- Calculate distance in miles using Haversine formula (when both coords available)
      CASE 
      ...
      -- Proximity tier: distance-based when coords available, else city/region/country fallback
      -- Online-only coaches always get tier 6 (shown after nearby in-person coaches)
      CASE
        -- Online-only coaches (no in-person) go to tier 6
        WHEN cp.online_available = TRUE AND cp.in_person_available = FALSE THEN 6
        -- Distance-based tiers (when both user and coach have coordinates)
        WHEN p_user_lat IS NOT NULL AND p_user_lng IS NOT NULL 
         AND cp.location_lat IS NOT NULL AND cp.location_lng IS NOT NULL THEN
          CASE
            ...
            ) <= 20 THEN 4  -- Far (10-20 miles)
            ELSE 5          -- Very far (20+ miles)
          END
        -- Fallback to city/region matching (when no coordinates)
      ...
      -- Location score (for backward compatibility)
      CASE
  ...
  ORDER BY 
    rc.location_tier ASC,                                              -- 1. Proximity FIRST (nearby before far)
    rc.is_sponsored DESC,                                              -- 2. Boost within tier (Amazon-style)
    (rc.location_score + rc.engagement_score + rc.profile_score) DESC, -- 3. Score within tier+boost
    rc.id ASC                                                          -- 4. Deterministic tiebreaker (prevents reordering)
  LIMIT p_limit;
END;
$$;
