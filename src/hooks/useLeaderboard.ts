...
export function useWeeklyLeaderboard(limit = 20) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const topClientIds = Object.entries(xpByClient).sort(([, a], [, b]) => b - a).slice(0, limit * 2).map(([id]) => id);
      if (topClientIds.length === 0) return [];
      
      // PRIVACY FIX: Only include clients who have opted in to leaderboards
      const { data: clients } = await supabase
        .from('client_profiles')
        .select('id, first_name, last_name, avatar_url, leaderboard_visible')
        .in('id', topClientIds)
      ...
      // Filter to only visible clients and limit
      const visibleClientIds = new Set(clients?.map(c => c.id) || []);
      const filteredTopClientIds = topClientIds.filter(id => visibleClientIds.has(id)).slice(0, limit);
      
      return filteredTopClientIds.map((clientId, index) => {
        const client = clients?.find(c => c.id === clientId);
        return { 
          client_id: clientId, 
          total_xp: xpByClient[clientId], 
          current_level: levelMap[clientId] || 1, 
...
export function useAllTimeLeaderboard(limit = 20) {
  return useQuery({
    ...
    queryFn: async () => {
      // First get clients who are visible on leaderboard
      const { data: visibleClients, error: clientsError } = await supabase
        .from('client_profiles')
        .select('id')
        .eq('leaderboard_visible', true);
      ...
      // PRIVACY FIX: Only get XP for visible clients
...
export function useMyRank() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Count how many visible users have more XP
      const { count: higherCount } = await supabase
        .from('client_xp')
        .select('*', { count: 'exact', head: true })
        .in('client_id', visibleClientIds.length > 0 ? visibleClientIds : [''])
