...
export function PriceSummary({ tier, billingInterval }: PriceSummaryProps) {
  ...
  // Map tier to pricing tier
  const pricingTier = tier as SubscriptionTier;
  const price = pricing.getSubscriptionPrice(pricingTier, billingInterval);
  const savings = pricing.getSubscriptionSavings(pricingTier);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-50 rounded-lg p-4 space-y-3">
        <div className="flex justify-between items-center">
          ...
          <p className="font-semibold text-gray-900">{pricing.formatPrice(price)}</p>
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <p className="font-medium text-gray-900">{pricing.formatPrice(price)}</p>
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <p className="font-bold text-gray-900 text-lg">{pricing.formatPrice(price)}</p>
        </div>
      </div>
      ...
      {billingInterval === "yearly" && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-3">
          <p className="text-sm text-green-800 font-medium">
            🎉 You're saving {pricing.formatPrice(savings)} with annual billing!
          </p>
        </div>
      )}
    </div>
