...
import { GripVertical, X, Clock, Video, ExternalLink, Dumbbell } from "lucide-react";
...
const SortableExerciseItem = ({ exercise, onUpdate, onRemove }: SortableExerciseItemProps) => {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex items-center gap-3 p-3 bg-card rounded-xl border border-border ${isDragging ? "shadow-glow" : ""}`}
    >
      <button
        {...attributes}
        {...listeners}
