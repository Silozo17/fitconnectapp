  ...
  return (
    <div className="space-y-2">
      {entries.map((entry) => {
        ...
        const displayName = entry.first_name && entry.last_name ? `${entry.first_name} ${entry.last_name}` : entry.first_name || 'Anonymous';
        ...
        return (
          <div key={entry.client_id} className={cn('flex items-center gap-4 p-3 rounded-lg border transition-all', getRankBg(entry.rank || 0), isMe && 'ring-2 ring-primary')}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-muted-foreground">Level {entry.current_level} • {getLevelTitle(entry.current_level)}</div>
            </div>
